    ...
    Func<WebBridgePackageFile, Task<byte[]>> downloadFile)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        string packageSegment = SanitizeSegment(manifest.PackageId);
        string packageRoot = Path.Combine(rootDirectory, "Packages", packageSegment);
        string manifestPath = Path.Combine(rootDirectory, "Manifests", $"{packageSegment}.json");
        string draftPath = Path.Combine(rootDirectory, "ConfigDrafts", $"{packageSegment}.json");
    ...
    static string SanitizeSegment(string value)
    {
        ...
        if (string.IsNullOrWhiteSpace(sanitized) ||
            ...
            string.Equals(sanitized, "..", StringComparison.Ordinal))
            throw new InvalidOperationException("PackageId contains an unsafe path segment.");
