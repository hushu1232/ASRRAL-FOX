...
#pragma warning restore CS0414
        ...
        [Header("Idle Behaviors — Data-Driven (ScriptableObject)")]
        [Tooltip("Assign IdleBehaviorDef assets here. When non-empty, replaces hardcoded behaviors.\nCreate via: Assets → Create → AstralFox → Idle Behavior")]
        [SerializeField]
        private IdleBehaviorDef[] _idleBehaviorDefs;

        [Header("Idle Behaviors — Legacy (fallback)")]
        ...
        // Runtime index into _idleBehaviorDefs (only used when data-driven mode is active)
        private IdleBehaviorDef _currentBehaviorDef;

        private bool UseDataDrivenBehaviors =>
            _idleBehaviorDefs != null && _idleBehaviorDefs.Length > 0;
        ...
        private void StartRandomBehavior()
        {
            ...
            if (UseDataDrivenBehaviors)
            {
                StartRandomDataDrivenBehavior();
                return;
            }

            // ── Legacy fallback: hardcoded weighted random ─────
            float rand = UnityEngine.Random.value;
            if (rand < 0.16f) _currentIdleBehavior = IdleBehavior.Scratch;
            else if (rand < 0.30f) _currentIdleBehavior = IdleBehavior.LookAround;
            else if (rand < 0.42f) _currentIdleBehavior = IdleBehavior.Stretch;
            else if (rand < 0.52f) _currentIdleBehavior = IdleBehavior.Sneeze;
            else if (rand < 0.62f) _currentIdleBehavior = IdleBehavior.ShakeBody;
            else if (rand < 0.70f) _currentIdleBehavior = IdleBehavior.TiltHead;
            else if (rand < 0.77f) _currentIdleBehavior = IdleBehavior.Wiggle;
            else if (rand < 0.84f) _currentIdleBehavior = IdleBehavior.Nuzzle;
            else if (rand < 0.90f) _currentIdleBehavior = IdleBehavior.FlopEars;
            else if (rand < 0.95f) _currentIdleBehavior = IdleBehavior.Bounce;
            else if (rand < 0.98f) _currentIdleBehavior = IdleBehavior.WavePaw;
            else _currentIdleBehavior = IdleBehavior.ChaseTail;

            Debug.Log($"[FoxAnimationController] Idle behavior (legacy): {_currentIdleBehavior}");
        }

        private void StartRandomDataDrivenBehavior()
        {
            // Weighted random selection from ScriptableObject array
            float totalWeight = 0f;
            foreach (var def in _idleBehaviorDefs)
                totalWeight += Mathf.Max(0.01f, def.weight);

            float rand = UnityEngine.Random.value * totalWeight;
            float cumulative = 0f;
            foreach (var def in _idleBehaviorDefs)
            {
                cumulative += Mathf.Max(0.01f, def.weight);
                if (rand <= cumulative)
                {
                    _currentBehaviorDef = def;
                    Debug.Log($"[FoxAnimationController] Idle behavior (data-driven): {def.behaviorName}");
                    return;
                }
            }
            // Fallback: pick first
            _currentBehaviorDef = _idleBehaviorDefs[0];
        }
        ...
        {
            // Data-driven path: use ScriptableObject curves
            if (UseDataDrivenBehaviors && _currentBehaviorDef != null)
            {
                DriveDataDrivenBehavior();
                return;
            }
        ...
        /// <summary>
        /// Generalized behavior driver using ScriptableObject animation curves.
        /// Replaces 12 hardcoded DriveXxx() methods when _idleBehaviorDefs is assigned.
        /// </summary>
        private void DriveDataDrivenBehavior()
        {
            float duration = Mathf.Max(0.1f, _currentBehaviorDef.duration);
            float t = Mathf.Clamp01(_idleBehaviorTimer / duration);
            _currentBehaviorDef.Evaluate(t, _driver);

            if (t >= 1f)
            {
                EndDataDrivenBehavior();
            }
        }

        private void EndDataDrivenBehavior()
        {
            _currentBehaviorDef = null;
            _idleBehaviorTimer = 0f;
            _nextBehaviorScheduledTime = UnityEngine.Random.Range(_idleBehaviorIntervalMin, _idleBehaviorIntervalMax);
