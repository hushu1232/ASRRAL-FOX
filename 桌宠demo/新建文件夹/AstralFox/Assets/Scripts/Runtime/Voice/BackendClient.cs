...
namespace AstralFox.Voice
{
    ...
    public sealed class BackendClient : MonoBehaviour, IVoicePipeline
    {
        ...
        private float _reconnectBaseDelay = 2f;     // base delay in seconds

        [SerializeField, Range(1f, 10f)]
        private float _reconnectMaxDelay = 60f;     // cap at 60 seconds

        [SerializeField, Range(1f, 2f)]
        private float _reconnectBackoffMultiplier = 2f; // exponential multiplier

        [SerializeField, Range(0f, 1f)]
        private float _reconnectJitter = 0.3f;      // random jitter (±30%)

        [SerializeField, Range(1f, 20f)]
        private int _maxReconnectAttempts = 10;     // give up after N failed attempts
...
#pragma warning restore CS0414
        ...
        private void Update()
        {
            ...
            else
            {
                ...
                if (_reconnectTimer >= _currentReconnectDelay)
                {
                    _reconnectTimer = 0f;
                    if (_reconnectAttempts < _maxReconnectAttempts)
                        _ = ConnectAsync();
                    else
                        Debug.LogWarning($"[BackendClient] Max reconnect attempts ({_maxReconnectAttempts}) reached. Giving up.");
                }
            }
        }
        ...
        public async Task ConnectAsync()
        {
            ...
            try
            {
                ...
                _currentReconnectDelay = _reconnectBaseDelay; // reset to base delay
                _pingTimer = 0f;
                _sendTimer = 0f;
            ...
            {
                _reconnectAttempts++;
                // Exponential backoff: baseDelay * multiplier^attempts + random jitter
                float exponentialDelay = _reconnectBaseDelay * MathF.Pow(_reconnectBackoffMultiplier, _reconnectAttempts - 1);
                float jitter = exponentialDelay * _reconnectJitter * (UnityEngine.Random.value * 2f - 1f);
                _currentReconnectDelay = MathF.Min(exponentialDelay + jitter, _reconnectMaxDelay);

                Diagnostics.DiagnosticBus.Warn("BackendClient",
                    $"Connection failed (attempt {_reconnectAttempts}/{_maxReconnectAttempts}, " +
                    $"next retry in {_currentReconnectDelay:F1}s): {ex.Message}");
